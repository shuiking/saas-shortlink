...
public class ShortLinkStatsServiceImpl implements ShortLinkStatsService {
    ...
    public ShortLinkStatsRespDTO oneShortLinkStats(ShortLinkStatsReqDTO requestParam) {
        ...
        rangeDates.forEach(each -> {
            Optional<LinkAccessStatsDO> optionalItem = listStatsByShortLink.stream()
                    .filter(item -> Objects.equals(each, DateUtil.formatDate(item.getDate())))
                    .findFirst();
            ShortLinkStatsAccessDailyRespDTO accessDailyRespDTO = optionalItem.map(item ->
...
//        TODO 修改
        rangeDates.forEach(each -> {
            Optional<LinkAccessStatsDO> optionalItem = listStatsByGroup.stream()
                    .filter(item -> Objects.equals(each, DateUtil.formatDate(item.getDate())))
                    .findFirst();
