package com.lk.shortlink.gateway.filter;//package com.lk.shortlink.gateway.filter;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.lk.shortlink.gateway.config.FilterConfig;
import com.lk.shortlink.gateway.dto.GatewayErrorResult;
...
/**
 * SpringCloud Gateway Token 拦截器,检验token
 */
@Component
            ...
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {
                ...
                if (StringUtils.hasText(username) && StringUtils.hasText(token) && (userInfo = stringRedisTemplate.opsForHash().get("short-link:login:" + username, token)) != null) {
                    ...
                    ServerHttpRequest.Builder builder = exchange.getRequest().mutate().headers(httpHeaders -> {
                        ...
                        try {
                            httpHeaders.set("realName", URLEncoder.encode(userInfoJsonObject.getString("realName"),"UTF-8"));
                        } catch (UnsupportedEncodingException e) {
                            System.out.println("编码设置异常");
                            e.printStackTrace();
                        }
                    });
